...
public class ImageTemp {
     ...
     * @throws IOException
     */
    public ImageTemp(InputStream input, ImageType sourceType) throws IOException {
     ...
     * @param img
     */
    public ImageTemp(BufferedImage img, ImageType sourceType) {
        this.img = img;
        this.sourceType = (sourceType == null ? ImageType.UNKNOWN : sourceType);
    }
    ...
    /**
     * Generate a new ImageTemp object resized to a specific width, maintaining
     * the same aspect ratio of the original
     *
     * @param width
     * @return ImageTemp scaled to new width
     */
    public ImageTemp getResizedToWidth(int width) {
        if (width > getWidth())
        ...
        return new ImageTemp(resizedImage, sourceType);
    }

    /**
     * Generate a new ImageTemp object cropped to a new size
     *
     ...
     * @return ImageTemp cropped to new dimensions
     */
    public ImageTemp crop(int x1, int y1, int x2, int y2) {
        ...
        return new ImageTemp(cropped, sourceType);
    }
     ...
     * @return ImageTemp cropped and resized to a square
     */
    public ImageTemp getResizedToSquare(int width, double cropEdgesPct) {
        ...
        ImageTemp cropped = crop(x1, y1, x2, y2);

        // now resize. we do crop first then resize to preserve detail
        ImageTemp resized = cropped.getResizedToWidth(width);
        cropped.dispose();
     ...
     * @return New ImageTemp object post-softening, unless softenFactor == 0, in which
     * case the same object is returned
     * @throws NotImplementedException Not implemented in this class
     */
    public ImageTemp soften(float softenFactor) {
        if (softenFactor == 0f)
            return this;
        else {
            ...
            return new ImageTemp(cOp.filter(img, null), sourceType);
        }
    }
